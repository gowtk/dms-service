...
public class DocumentProcessor {
	...
	private static String sqsRegion = "us-east-2";

	// SNS - Simple Notification Service
	private static String snsRegion = "us-east-2";

	public String s3BucketName() {
		return bucketName;
	}

	public static AmazonS3 initiateS3Clinet() {
	...
	public static AmazonTextract initiateTextractClient() {
	...
	public static AmazonSQS initiateSQSClient() {
	...
	public static AmazonSNS initiateSNSClient() {
	...
	public static void main(String[] args) throws Exception {
		...
		String document = "5d9b817b57b8991b102a2466.pdf";
		String bucket = bucketName;
		String roleArn = "textract_sns";

		sns = initiateSNSClient();
		sqs = initiateSQSClient();
		textract = initiateTextractClient();
	...
	static void CreateTopicandQueue() {
		...
		Policy policy = new Policy().withStatements(
				new Statement(Effect.Allow).withPrincipals(Principal.AllUsers).withActions(SQSActions.SendMessage).withResources(new Resource(sqsQueueArn))
	...
	private static void StartDocumentTextDetection(String bucket, String document) throws Exception {
		...
		StartDocumentTextDetectionRequest req = new StartDocumentTextDetectionRequest()
				.withDocumentLocation(new DocumentLocation().withS3Object(new S3Object().withBucket(bucket).withName(document))).withJobTag("DetectingText")
	...
	private static void GetDocumentTextDetectionResults() throws Exception {
		...
		while (finished == false) {
			GetDocumentTextDetectionRequest documentTextDetectionRequest = new GetDocumentTextDetectionRequest().withJobId(startJobId)
	...
	private static void StartDocumentAnalysis(String bucket, String document) throws Exception {
		...
		StartDocumentAnalysisRequest req = new StartDocumentAnalysisRequest().withFeatureTypes("TABLES", "FORMS")
				.withDocumentLocation(new DocumentLocation().withS3Object(new S3Object().withBucket(bucket).withName(document))).withJobTag("AnalyzingText")
				.withNotificationChannel(channel);
	...
	private static void GetDocumentAnalysisResults() throws Exception {
		...
		while (finished == false) {
			GetDocumentAnalysisRequest documentAnalysisRequest = new GetDocumentAnalysisRequest().withJobId(startJobId).withMaxResults(maxResults)
